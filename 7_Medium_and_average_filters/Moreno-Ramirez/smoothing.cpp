...
void averageFilter(Mat img){
	Mat mask(7, 7, CV_8UC1, Scalar(1));
	Mat imageOut = img.clone();
	int sum, N=round(mask.rows/2), mi=0, mj=0, neighbors=0, elements=Num(mask);
...
void medianFilter(Mat img){
	Mat mask(7, 7, CV_8UC1, Scalar(1)), imageOut = img.clone();
